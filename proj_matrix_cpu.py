...
def cache_spots(nx, ny, nspec, nwave, p, wavelengths):
    ...
    for ispec in range(nspec):
        ...
        ghc = p['GH'][:,:,ispec,:]
        mspots = multispot(pGHx, pGHy, ghc)
        spots[ispec] = mspots
    return spots #for now return spots on the cpu
...
#cpu
xc, yc = corners
A, ymin, xmin = projection_matrix(ispec, nspec, iwave, nwave, spots, corners)
A_cpu = A
print("np.max(A_cpu)", np.max(A_cpu))

np.save('A_cpu.npy', A_cpu)

np.save('cpu_spots.npy', spots)
